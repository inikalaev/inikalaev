# Hi, I'm Ivan 👋

Ruby backend developer. Building reliable services and exploring new stacks.

## Stack
- **Backend**: Ruby, Ruby on Rails, PostgreSQL
- **Growing into**: Go, React
- **Tools**: Git, Docker, REST APIs

## Projects
- **[provenance](https://github.com/inikalaev/provenance)** — Drop-in audit trail for Rails. Records user actions and ActiveRecord model changes, groups them per request and transaction, redacts sensitive data, and ships structured audit events to any sink through configurable hooks.
- **[database-seed-cli](https://github.com/inikalaev/database-seed-cli)** — Go CLI for generating relationally consistent synthetic data for PostgreSQL. Reads a live schema, infers fill mechanisms per column, and emits a ready-to-run SQL seed script respecting FK dependencies.

## Currently
- Building backend services
- Expanding into Go and frontend
- Exploring open source contributions

## Contact
✉️ ivan_n6_20@icloud.com  
📝 [habr.com/ru/users/ShonZander](https://habr.com/ru/users/ShonZander/)
